...
class GameModel : NSObject {
    ...
    func insertTile(at location: (Int, Int), value: Int) {
        print("insertTile \(location) \(value)")
        let (x, y) = location
    ...
    func insertTileAtRandomLocation(withValue value: Int) {
        print("In insertTileAtRandomLocation \(value)")
    ...
    func getPowerValueOfTile(v:Int){
        let valueOfTile = Double(v)
        let startValue = 2048
        let power = getExpValue(num: valueOfTile)
        let powerofStartValue = getExpValue(num: Double(startValue))
        if power == 6 {
            score += Int(valueOfTile)
        }else{
            let tileValuePowDiff = powerofStartValue - power
            let valueToadd = pow(2, Double(tileValuePowDiff+1))
            score += Int(valueToadd)
        }
    }
    
    func getExpValue(num:Double) -> Int{
        var powerCount = 0.0
        while pow(2, powerCount) < num{
            powerCount += 1
        }
        print("power of tile value \(powerCount)")
        return Int(powerCount)
    }
